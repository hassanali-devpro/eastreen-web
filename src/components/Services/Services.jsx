                ...
                <ScrollTrigger onEnter={() => setIsEnter(true)} onExit={() => setIsEnter(false)}>
                    <div className="grid gap-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {data.services.map((service, index) => (
                            <a href={service.web} target="_blank" rel="noopener noreferrer" className={`${isEnter ? 'animate-slide-up' : 'opacity-0 translate-y-10'} p-8 text-center transform transition-transform duration-300 hover:shadow-2xl hover:-translate-y-3 rounded-3xl border-[0.5px] border-gray-300`} key={index}>
